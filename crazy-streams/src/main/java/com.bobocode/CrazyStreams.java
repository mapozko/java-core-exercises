...
public class CrazyStreams {
    ...
    public List<Account> findAccountsByBirthdayMonth(Month birthdayMonth) {
        return accounts.stream()
                .filter(x -> x.getBirthday().getMonth().equals(birthdayMonth))
    ...
    public Map<Boolean, List<Account>> partitionMaleAccounts() {
        return accounts.stream()
                .collect(Collectors.partitioningBy(x -> x.getSex().equals(Sex.MALE)));
    }
    ...
    public Map<String, List<Account>> groupAccountsByEmailDomain() {
        return accounts.stream()
                .collect(Collectors.groupingBy(x -> x.getEmail().substring(x.getEmail().lastIndexOf("@") + 1)));
    }
    ...
    public int getNumOfLettersInFirstAndLastNames() {
        return accounts.stream()
                .mapToInt(x -> x.getFirstName().length() + x.getLastName().length())
                .sum();
    }
    ...
    public List<Account> sortByFirstAndLastNames() {
        return accounts.stream()
                .sorted(Comparator.comparing(x -> x.getFirstName() + x.getLastName()))
                .collect(Collectors.toList());
    }
    ...
    public boolean containsAccountWithEmailDomain(String emailDomain) {
        return accounts.stream()
                .anyMatch(x -> x.getEmail().contains(emailDomain));
    }
    ...
    public Map<String, BigDecimal> collectBalancesByIdForAccountsCreatedOn(int year) {
        return accounts.stream()
                .filter(x -> x.getCreationDate().getYear() == year)
    ...
    public Map<Month, BigDecimal> groupTotalBalanceByCreationMonth() {
        return accounts.stream()
                .collect(
                        Collectors.groupingBy(
                                acc -> acc.getCreationDate().getMonth(),
    ...
    public Map<Character, Long> getCharacterFrequencyInFirstNames() {
        return accounts.stream()
                ...
                .flatMap(name -> name.chars().mapToObj(x -> (char)x))
